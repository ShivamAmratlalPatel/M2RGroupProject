...
\newline The algorithm is also proved to achieve the upper-bounded regret by $O\left(\sqrt{\gamma_{i}T}\log{T}\right)$ as the lower bound is $\Omega(\sqrt{T})$\citep{Liu_Lee_Shroff_201}. As well, the algorithm is shown to be more weighted for new data rather than old data.
